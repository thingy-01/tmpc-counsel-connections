...
- September 3: consuming a one-use attorney token on GET lets email security scanners invalidate a fresh link before the recipient opens it. Keep GET read-only and return a private, nonce-restricted page that auto-submits the redemption POST in a browser, preserving direct entry without a required confirmation click.
