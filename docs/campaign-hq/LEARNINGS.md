...
- September 3: a confirmation page with `Referrer-Policy: no-referrer` can submit a native same-origin form with the literal `Origin: null`. Accept that opaque-origin shape only when Fetch Metadata proves a same-origin document navigation (`Sec-Fetch-Site: same-origin`, `Sec-Fetch-Mode: navigate`, `Sec-Fetch-Dest: document`); continue to require exact matching for explicit origins and reject foreign-origin posts.
