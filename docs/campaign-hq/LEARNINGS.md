...
- September 3: the attorney confirmation page added an unreliable second browser step and its Continue action failed in production. Redeem the one-use token on the emailed GET and redirect directly to `/attorney/schedule`; keep the callback response private and use the configured public origin behind deployment proxies.
