...
- September 3: consuming a one-use attorney token on every GET lets email security scanners invalidate a fresh link, and scanners may execute JavaScript on returned pages. Redeem GET only when Fetch Metadata identifies a user-activated top-level document navigation; return an inert no-script POST fallback for scanner-like or metadata-free GETs. Redirecting email wrappers and older browsers may omit `Sec-Fetch-User` and show the button, while a scanner that deliberately spoofs the full browser signal can still consume the link.
